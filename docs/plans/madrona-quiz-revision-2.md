...
**There are two published-question read paths, not one, and the second
is an Edge Function.** The browser reads through
...
Both name explicit column lists, and the two lists are independent
literals that must agree with no mechanism holding them together. The
Edge Function's question query casts its result with an unchecked
...
build time. Consequence: the loader is inside PR 1's scope, and the two
column lists want one owner.
...
import hop away in `dependencies.ts`, and it did not. The procedure
that does: search the whole functions tree for the table name and
follow every importer of whatever it finds. Recorded here because the
weaker check looked sufficient and was not.

**The demo-mode read function reads no question content rows.** It
reads the admin status view and redemption rows only (`Verified by:`
...
- **Every published-question read path selects the same column set.**
  ...
  constant both consume, which converts this from a rule someone has to
  remember into one that cannot be broken by editing one site.

This section carries five invariants rather than the two-to-four the
per-level guidance suggests. The fifth is the direct product of a
review finding on this plan — the loader was originally listed as not
touched — and naming it is what keeps the same omission from recurring
at the next read path.
...
leaves the invariant false for text the parser does not recognize as a
link at all. An entry with an unclosed link — a missing closing
parenthesis is the realistic authoring slip — matches nothing, falls to
the plain-text path, and prints its address on screen. That is the
bare-address outcome the second cross-cutting invariant forbids, and no
amount of care inside the link branch prevents it. Rejecting the entry
at authoring time does.
...
The new column is added to that constant, so both paths pick it up
from one edit. The row-to-runtime mapper carries the value into the
shared question shape, treating a null or absent value as the empty
list.
...
behavior are unchanged. It is in this contract because it reads
question rows, not because it does anything with sources.
...
content that predates the rule or arrives from a path validation did
not cover.
...
question field with its parsing and validation, both mappers, and the
shared column constant with the two read paths that consume it — the
browser read and the completion function's loader. Ships nothing a
player can see: no content has sources, so every new code path is inert
in production.
...
- [shared/events/published.ts](/shared/events/published.ts) — consumes
  the shared column constant of C4 instead of its own literal list
...
- [supabase/functions/complete-game/index.ts](/supabase/functions/complete-game/index.ts)
  ...
  — grading, entitlement, and idempotency are unchanged. The function
  is nonetheless affected through the loader it imports, which is in
  the Modify list above; "not touched" here means these two files, not
  the function's blast radius
...
database tests; the shared column constant and both read paths that
consume it. Within PR 2: the component and all three render sites in
one commit; the results gate and its rewritten tests; admin authoring;
styling. Within PR 3: the seed transcription; documentation; the
close-out commit that flips Status and deletes the scoping doc. The
copy deck is not here — PR 0 lands it. Review-fix commits stay
distinct.
...
the edge-function suite, and the database test gate PR 1. The
mobile-width and contrast inspections gate PR 2. The link check, the
live play-through, and the post-release admin smoke on PR 1's promotion
all gate PR 3 — the play-through cannot run before content is
published, and publishing content onto an unsmoked publish routine is
the ordering this split exists to prevent.

**The admin end-to-end suite gates PR 1 and PR 2 both, and must be run
deliberately.** It is local-only by design: excluded from CI and
excluded from the full local validation flow (`Verified by:`
[docs/dev.md:591-609](/docs/dev.md), which names it as the command to
use when a change can affect publish/unpublish behavior, draft
persistence, or the admin UI, and states both exclusions). PR 1
replaces the publish routine, which is publish behavior. PR 2 changes
the admin UI and the draft-persistence path. Neither is covered by any
gate that runs on its own, so both name this command explicitly or it
never runs.
