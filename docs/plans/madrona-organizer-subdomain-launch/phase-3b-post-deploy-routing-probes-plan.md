...
the parent's Status flip.** Which flip depends on the parent's own
gate — the composed production journey, a different check from this
phase's probe run and one that does not ride on it. Per the parent's
"Status lifecycle and close-out": if that journey has already passed
when this PR is opened, the parent goes straight to `Landed` here and
this PR deletes the scoping doc; if it has not, the parent goes to
`In progress pending organizer-host verification` here and reaches
`Landed` in a follow-up doc-only commit. Read the parent before
opening this PR rather than assuming either branch.

Two ordering bounds make this PR last — the data-hygiene rename and
phase 2's documentation each land no later than it — and the parent
states both. If either is still outstanding when this PR is ready,
that is a signal to hold this PR, not to carry the parent's flip
anyway.
...
| `madrona-organizer-subdomain-launch.md` | the parent's Status flip, in this phase's implementing PR — to `Landed` or to `In progress pending organizer-host verification`, per the branch above |
