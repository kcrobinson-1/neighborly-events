...
| G8 | PR CI covers the pre-release change set at a meaningful depth | For non-doc changes, lint, unit, Deno function tests (over every function in `supabase/functions/*/`), local Supabase integration + pgTAP, attendee-trusted-backend Playwright smoke, and both web (`build:web`) and site (`build:site`) builds pass on the release candidate commit via `.github/workflows/ci.yml`; the `release.yml` workflow then promotes Supabase migrations + Vercel deploys, and `production-admin-smoke.yml` validates the deployed admin surface. Docs-only pull requests still produce the required CI check but intentionally short-circuit heavy validation, and markdown/docs-only commits to `main` do not trigger production release. Any intentional gap is a known item in `backlog.md` | [testing.md — Where Tests Should Run](/docs/testing.md) and [backlog.md Tier 2](/docs/backlog.md) |
...
- **Promote redemption operator path to its own gate (or fold it into
  G3 with named evidence).** Today G3 names the redemption operator
  path in its evidence requirement but the canonical evidence —
  `production-admin-smoke.yml` — does not exercise it. Either
  `production-admin-smoke.yml` should grow a redemption phase and the
  G3 wording stays as-is, or the redemption path graduates to a
  separate gate with its own evidence. Decide before the first event
  that ships volunteer redemption.
- **Add a CI-coverage gate for the full `supabase/functions/*/`
  ...
  surfaced four functions (`read-demo-event`, `get-redemption-status`,
  `redeem-entitlement`, `reverse-entitlement-redemption`) silently
  outside the deno-check step. Decide whether to (a) tighten G8's
  wording to require explicit coverage of every function, or (b)
  treat per-function CI coverage as a per-pass cleanliness check
  rather than a gate.
