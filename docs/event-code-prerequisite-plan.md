...
takes priority over this work. That migration renames `quiz_*` → `game_*` and
`raffle_*` → `entitlement_*` across schema, edge functions, and frontend
contracts. This document is intentionally written against the **pre-migration
names** so it stays diff-friendly with the current codebase. When the
migration's Phase 2 (DB) and Phase 3 (edge function) PRs land, every name in
the table below must be rewritten in this plan as part of that migration's
doc-currency gate — do **not** start implementation against stale names.

| Pre-migration name (used here) | Post-migration name |
| --- | --- |
| `quiz_event_drafts` | `game_event_drafts` |
| `quiz_events` | `game_events` |
| `raffle_entitlements` | `entitlements` |
...
| `enforce_quiz_event_draft_event_code_lock` | `enforce_game_event_draft_event_code_lock` |
| `complete-quiz` (edge function / test fixtures) | `complete-game` |
| `apps/web/src/lib/quizApi.ts` | renamed per Phase 4 frontend module rename |

If implementation starts after the migration lands, prefer the post-migration
names directly and treat the column on the right as authoritative.

## Summary

Introduce a new `event_code` attribute on every quiz event: a 3-letter prefix that the system generates at event creation and that an admin or organizer can override before publish. Rewrite entitlement code generation from `MMP-XXXXXXXX` to `<event_code>-NNNN` (4 numeric digits), producing codes shaped like `ABC-1234` — short enough for an attendee to read aloud and an agent to type on a keypad in seconds, with the prefix acting as inline event validation.
...
- Every `quiz_event_drafts` row and every `quiz_events` row has a non-null `event_code` matching `^[A-Z]{3}$` (exactly 3 uppercase letters).
...
- Entitlement codes produced by `complete_quiz_and_award_entitlement` take the form `<event_code>-NNNN` where `NNNN` is exactly 4 numeric digits (0000–9999). Numeric was chosen over hex so an agent redeeming the code at an event can type it on a number pad without mode-switching. The 3-letter prefix acts as inline event validation — the full `ABC-1234` string is what the attendee reads and the agent hears.
- Verification codes are unique per event (`unique (event_id, verification_code)` on `raffle_entitlements`); the RPC retries on collision.
...
- New column on `quiz_event_drafts`: `event_code text` with check constraint `event_code ~ '^[A-Z]{3}$'`. `NOT NULL` after backfill.
- New column on `quiz_events` (published projection): `event_code text`, same constraint, `NOT NULL`.
- Unique index on `quiz_event_drafts(event_code)`.
- Unique index on `quiz_events(event_code)`.
...
- New unique constraint on `raffle_entitlements(event_id, verification_code)`. This is the per-event uniqueness invariant the redemption MVP previously deferred to this prerequisite, and it is what makes the 4-digit numeric token safe to reuse across events while remaining unambiguous within an event.

### Locking

- New DB trigger `enforce_quiz_event_draft_event_code_lock` on `quiz_event_drafts`, modelled after `enforce_quiz_event_draft_slug_lock` (migration `20260415000000_add_quiz_event_draft_slug_lock_trigger.sql`). Raises exception `event_code_locked` if `OLD.event_code IS DISTINCT FROM NEW.event_code` when `OLD.live_version_number is not null`.
...
- `generate_neighborly_verification_code(p_event_code text)` returns `p_event_code || '-' || lpad((('x' || encode(gen_random_bytes(2), 'hex'))::bit(16)::int % 10000)::text, 4, '0')`.
  - Produces a 4-digit numeric token in `[0000, 9999]` using cryptographically strong randomness from `pgcrypto` (16 bits of entropy mod 10,000; the small bias of mod over 16,384/10,000 is acceptable for raffle token generation).
...
- `complete_quiz_and_award_entitlement` fetches `event_code` from `quiz_events` by `event_id` and passes it to `generate_neighborly_verification_code`. The insert into `raffle_entitlements` happens inside a bounded retry loop (up to 10 attempts) that catches the `unique_violation` on `(event_id, verification_code)` and tries a fresh token. If all 10 attempts collide (vanishingly unlikely below ~9,000 issued codes), the RPC raises `entitlement_code_exhausted`, which the edge function maps to a 503 `{ outcome: "error", result: { code: "entitlement_code_exhausted" } }` so the operator can decide whether to widen the token or rotate the event_code. If the event is missing or has no code (should be impossible after backfill + NOT NULL), the RPC raises `event_code_missing`, which the edge function maps to a 500 `{ outcome: "error", result: { code: "server_error" } }`.
- Frontend mock `createVerificationCode()` in `apps/web/src/lib/quizApi.ts` is rewritten to accept `eventCode` and return `${eventCode}-${fourDigits}`, where `fourDigits` is a zero-padded random integer in `[0, 9999]`.
...
- Label: "Event code"
- Helper: "3-letter prefix used in raffle entry codes (e.g. `ABC-1234`). Auto-generated — change it if you want a more memorable prefix. Cannot be changed after the event is published."
...
1. `alter table quiz_event_drafts add column event_code text` (nullable).
2. `alter table quiz_events add column event_code text` (nullable).
3. Add `check (event_code is null or event_code ~ '^[A-Z]{3}$')` to both tables.

### Migration B — `20260418010000_backfill_event_code.sql`
1. Backfill `quiz_event_drafts` deterministically by `created_at asc, id asc`: the Nth row gets the Nth 3-letter sequence starting from `AAA` (`AAA`, `AAB`, …, `AAZ`, `ABA`, …, `ZZZ`). The conversion from row number to 3-letter code is a small PL/pgSQL helper that treats the row number as a base-26 integer with digits in `A-Z`.
2. Mirror the values into `quiz_events` for already-published rows by joining on `id`.
3. `alter table quiz_event_drafts alter column event_code set not null`.
4. `alter table quiz_events alter column event_code set not null`.
5. `create unique index quiz_event_drafts_event_code_key on quiz_event_drafts(event_code)`.
6. `create unique index quiz_events_event_code_key on quiz_events(event_code)`.
...
1. Create trigger function `enforce_quiz_event_draft_event_code_lock` mirroring the slug-lock trigger.
2. Attach to `quiz_event_drafts` for `before update`.
...
3. Add `alter table raffle_entitlements add constraint raffle_entitlements_event_code_unique unique (event_id, verification_code)`.
4. Replace the body of `complete_quiz_and_award_entitlement` so that:
   - It looks up `event_code` from `quiz_events` by `p_event_id`. Raise `event_code_missing` if null.
   - The insert into `raffle_entitlements` runs inside a bounded retry loop (up to 10 attempts): generate a new token, attempt insert, catch `unique_violation` on the new `(event_id, verification_code)` constraint, loop. If all 10 attempts collide, raise `entitlement_code_exhausted`.
...
5. Update `publish_quiz_event_draft` (defined in migration `20260410170000_add_quiz_authoring_publish_workflow.sql`) so the upsert into `quiz_events` carries `event_code` over from the draft. Without this step, newly-published events would have a null `event_code` and the NOT NULL constraint on `quiz_events.event_code` would fail the publish transaction.
...
1. pgTAP additions under `supabase/tests/`:
   ...
   - Unique-violation surfaces on both `quiz_event_drafts` and `quiz_events`.
   ...
   - `complete_quiz_and_award_entitlement` returns `verification_code` of the form `^<event_code>-[0-9]{4}$`.
   - Per-event uniqueness on `verification_code` enforced: pre-seed `raffle_entitlements` with rows occupying every code in `[0000, 9999]` for a given event, call the RPC, and assert it raises `entitlement_code_exhausted` (after 10 retries). With one slot free, the RPC succeeds.
2. Vitest additions:
...
3. Update `tests/supabase/functions/complete-quiz.test.ts` expectations (format assertions on `verification_code`, including the new retry-on-collision behavior).
...
- `npm run test:functions` — pgTAP (event_code constraints, trigger, code generator, complete-quiz RPC).
- `npm run build:web` — make sure the new admin form wiring compiles.
- Manual: create a draft, assign a code, publish, confirm the code input is disabled; complete a quiz and confirm the issued verification code starts with the event code.
...
- **Retry loop starvation.** Each retry runs inside the same advisory-lock-holding transaction as the existing `complete_quiz_and_award_entitlement` serialization. 10 attempts at the expected retry rate add milliseconds, not seconds, so holding the lock during retries is acceptable.
...
2. Revert Migration D: restore the zero-arg `generate_neighborly_verification_code`, the pre-feature body of `complete_quiz_and_award_entitlement`, the pre-feature `publish_quiz_event_draft` body (dropping the `event_code` copy-over step), and drop the `raffle_entitlements_event_code_unique` constraint.
3. Revert Migration C: drop the trigger.
4. Revert Migration B: drop the unique indexes on `quiz_event_drafts.event_code` and `quiz_events.event_code`, drop `not null`, leave the `event_code` columns populated for forensic inspection.
