...
Current setup and release workflow still live in `dev.md`. System responsibilities and trust boundaries live in `architecture.md`. UX goals live in `experience.md`. Release gates that depend on test coverage live in [`release-readiness.md`](/docs/tracking/release-readiness.md), which links back into the Todo List in this file rather than duplicating it.
