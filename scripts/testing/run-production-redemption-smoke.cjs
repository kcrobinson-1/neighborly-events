...
async function main() {
  ...
  logStep("Waiting for deployed redeem route readiness");
  await waitForRouteReady({
    pollMs,
    routeLabel: `/event/${eventSlug}/game/redeem`,
    routeUrl: `${baseUrl}/event/${eventSlug}/game/redeem`,
    timeoutMs,
  });

  logStep("Waiting for deployed redemptions route readiness");
  await waitForRouteReady({
    pollMs,
    routeLabel: `/event/${eventSlug}/game/redemptions`,
    routeUrl: `${baseUrl}/event/${eventSlug}/game/redemptions`,
    timeoutMs,
  });

  logStep("Running production redemption Playwright smoke suite");
  run("npx", [
    "playwright",
    "test",
    "-c",
    "playwright.production-redemption-smoke.config.ts",
  ]);
}

main().catch((error) => {
  console.error(`\nProduction redemption smoke failed.\n${error.message}`);
  process.exit(1);
});
