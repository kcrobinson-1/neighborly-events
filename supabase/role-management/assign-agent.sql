...
-- Assigns role = 'agent' for one user on one event. Idempotent on
...
begin
  ...
  values (v_user_id, v_event_id, 'agent', public.current_request_user_id())
  on conflict on constraint event_role_assignments_unique do nothing
  returning * into v_inserted;

  if v_inserted.id is null then
    raise notice 'Assignment already existed: user=% event=% role=agent (idempotent no-op)',
      v_user_id, v_event_id;
  else
    raise notice 'Assigned role agent: id=% user=% event=% created_at=%',
