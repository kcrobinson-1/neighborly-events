...
select ok(
  ...
  'first completion is entitlement eligible'
);

select is(
  (select count(*) from public.game_entitlements where event_id = 'test-event' and client_session_id = 'test-session'),
  1::bigint,
  'first completion creates exactly one entitlement row'
);

select is(
  (select count(*) from public.game_completions where event_id = 'test-event' and client_session_id = 'test-session'),
  1::bigint,
  'first completion creates exactly one completion row'
);

select is(
  (select first_completion_id::text from public.game_entitlements where event_id = 'test-event' and client_session_id = 'test-session'),
...
select ok(
  ...
  'a later request does not earn a second entitlement'
);
...
select ok(
  has_function_privilege(
    'service_role',
    'public.complete_game_and_award_entitlement(text, text, text, jsonb, integer, integer)',
    'EXECUTE'
  ),
