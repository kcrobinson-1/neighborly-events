-- Phase 3 terminology migration: move authoring JSON and publish projection
-- from the legacy draft field to the target entitlement field.

update public.game_event_drafts
set content = (content - 'raffleLabel')
  || case
    when content ? 'entitlementLabel' then '{}'::jsonb
    else jsonb_build_object('entitlementLabel', content ->> 'raffleLabel')
  end
where content ? 'raffleLabel';

update public.game_event_versions
set content = (content - 'raffleLabel')
  || case
    when content ? 'entitlementLabel' then '{}'::jsonb
    else jsonb_build_object('entitlementLabel', content ->> 'raffleLabel')
