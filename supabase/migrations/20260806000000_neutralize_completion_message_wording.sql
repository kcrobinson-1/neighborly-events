-- Neutralize reward wording in complete_game_and_award_entitlement's message field.
--
-- The Madrona reward-language rule bans "raffle" wording anywhere in
-- attendee-facing copy. The attendee UI does not render this RPC's `message`
-- field today, but the strings "You earned your raffle entry." / "You already
-- earned your raffle entry. This retake does not create another ticket."
-- (installed by 20260507010000_relax_event_code_lock_with_entitlements_guard.sql)
-- would surface verbatim if a future UI change starts displaying `message`.
--
-- The function body is otherwise byte-for-byte identical to the version
-- installed by 20260507010000; only the two `message` case expressions change:
--   new entitlement  → 'You earned your reward entitlement.'
--   existing         → 'You already earned your reward entitlement. This retake does not create another one.'
...
begin
  ...
  if found then
    ...
    select
      ...
      case
        when v_existing_completion.entitlement_awarded then 'You earned your reward entitlement.'
        else 'You already earned your reward entitlement. This retake does not create another one.'
      end;
  ...
  select
    ...
    v_entitlement_status = 'new',
    case
      when v_entitlement_status = 'new' then 'You earned your reward entitlement.'
      else 'You already earned your reward entitlement. This retake does not create another one.'
    end;
end;
$$;
