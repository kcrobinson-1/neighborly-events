 ...
 *
 * `newsletter` presence must track the event's feedback-surface
 * enablement — the `feedback_enabled_events` row seeded by the
 ...
 * same-origin href because the feedback route is owned by apps/site
 * across the proxy topology; listing `newsletter` for an event whose
 * feedback surface is not enabled produces a dead link. This
 * registry is the deliberately lightweight launch-mode conditional
 * (driver: Aug 11 Madrona launch push); if per-event content later
 * consolidates into a shared registry, fold this module into it.
 *
 * `donate.href` keeps donation destinations out of component code so
 * non-donating events stay unaffected and a URL swap is a content
 ...
 * block. The fixture only resolves under the Vite prototype
 * fallback, so the mirror never reaches production routes.
 */
...
export type CompletionCtaContent = {
  heading: string;
  /** Present only when the event's feedback surface is enabled. */
  newsletter?: CompletionCtaLink;
  /** Present only when the event has a donation destination. */
  donate?: CompletionCtaLink & { href: string };
};
