...
Player-facing copy for the five questions at
`madrona.us/musicintheplayfield`. This deck is the authority for
prompts, answer option labels, explanations, and source lines;
[`madrona-demo-game-content.ts`](./madrona-demo-game-content.ts) is
transcribed from it and must match it exactly.

Everything on this page is player-facing. There is no sponsor line on
the quiz and no sponsor attribution on any question.
...
**Link verification.** All eighteen links below were retrieved and
confirmed live on 2026-08-08, with titles and author attributions
matching what is written here. Re-verify at implementation time and
update this date. A dead citation is worse than no citation.
...
> a standard trick for selling lots. John E. Ayer, one of the property
...
> tree in the area. Parks historian Don Sherwood's files say there were
> "scarcely more than a few little madrona sprouts" in the park, and a
...
**Sources** — five entries, six links.
...
**Sources** — four entries, two links, two print books.
...
**Sources** — four entries, four links.
...
> builder." The campaign to change it began in November 1980, when
> activist Eddie Rye Jr. was hosting a program on Black radio station
> KYAC with Jesse Jackson as his guest and learned that cities around
...
> against them on November 30, 1983, and new signs started going up the
> next day. A crowd gathered to unveil the final one on January 15,
> 1984.

**Sources** — two entries, two links.
...
and no record of a plat ever renamed Madrona, so neither appears.
Accounts disagree on how many new street signs went up after the 1983
ruling, so question 5 says only that signs started going up the next
day and the last was unveiled on January 15, 1984, which every source
agrees on.
