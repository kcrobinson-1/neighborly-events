...
// while keeping the chunked-cookie format shared by apps/web and
// apps/site browser clients.
//
 ...
 * and `Secure` when `window.location.protocol === "https:"`. apps/site's
 * browser adapter consumes this same factory when `/auth/callback` is
 * proxied through the apps/web frontend origin. No `Domain=` attribute
 * means the cookie is host-only.
 ...
 * endpoint redirects with. Implicit flow keeps both real magic-link
 * sign-ins and the production smoke fixture working across the apps/web
 * and apps/site browser adapters.
 */
